...
interface GooglePlayPackageProps {
  googlePlayPackage: DataPackage
  selectedPackage: DataPackage | undefined
}

export default function GooglePlayPackage({
  googlePlayPackage,
  selectedPackage,
}: GooglePlayPackageProps) {
  const { ID, amount, status, badgeText } = googlePlayPackage
  const isMaintained = status === PackageStatus.MAINTENANCE
  const isSelected = ID && ID === selectedPackage?.ID

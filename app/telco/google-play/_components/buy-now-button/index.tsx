...
export default function BuyNowButton() {
  const { isLoading, selectedSupplier, onScrollToView } = useContext(GooglePlayContext)
  ...
  const goolePlayPackage = getValues('package')
  ...
  const handleCreateOrder = async (failureCallback: () => void) => {
    try {
      const response: any = await telcoAPI.createOrder({
        productID: ProductID.GOOGLEPLAY,
        dataPackage: goolePlayPackage,

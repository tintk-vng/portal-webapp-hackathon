...
const MAINTENANCE_BUTTON_NAME = 'Mua thẻ điện thoại'
...
export default function MaintenanceState({ telcoCode }: MaintenanceStateProps) {
  ...
  const handleClick = () => {
    commonUtil.trackEvent({
      ID: EVENT[AppID.POST_PAID].CLICK_MAINTENANCE_BUTTON,
      metaData: { cta_name: MAINTENANCE_BUTTON_NAME },
    })
    router.push(MAPPED_PATH[Domain.TELCO][AppID.PHONE_CARD]?.source || '')
  }
  ...
  return (
    <StateView
      ...
      } đang thực hiện bảo trì dịch vụ điện thoại trả sau trên Zalopay. Bạn vui lòng quay lại sau ít phút nhé.`}

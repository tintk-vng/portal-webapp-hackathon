import Image from '@/components/common/image'
import { SupplierGroup } from '@/types/bill/water'
import { useFormContext } from 'react-hook-form'
import { getBillDemo } from '../../../logo-mapping'

interface SupplierSelectProps {
  selectedSupplierGroup: SupplierGroup
}

export default function ExampleBill({ selectedSupplierGroup }: SupplierSelectProps) {
  const { getValues } = useFormContext()
  const selectedSupplier = getValues('supplier')

  const exampleBill = getBillDemo(
    parseInt(selectedSupplierGroup.ID) || 0,
    parseInt(selectedSupplier?.ID || '0')
  )

  if (!exampleBill) {
    return null
  }

  return (
    <div>
      <div className="mt-4 text-label-lg text-dark-300">Hoá đơn mẫu</div>

      <div className="mt-3 flex max-h-full max-w-full items-center justify-center">
        <Image
          className="h-auto w-full md:h-96 md:w-auto"
          src={exampleBill}
          width={0}
          height={0}
          alt="tra-cuu-thanh-toan-phi-chung-cu-online"
          loader={({ src }) => src}
        />
      </div>
    </div>
  )
}

...
const WATER_HOME_PATH = MAPPED_PATH[Domain.BILL][AppID.WATER]?.source || '/nuoc'
...
export default function BillDetails() {
  ...
  const { supplier, billInfo } = useWaterStore()
  ...
  if (commonUtil.isEmpty(supplier) || commonUtil.isEmpty(modeledBillInfo)) {
    redirect(WATER_HOME_PATH)
  }

  const handleCloseClick = () => {
    router.replace(WATER_HOME_PATH)
  }
  ...
  return (
    <div>
      <SupplierHeader title="Chi tiết hóa đơn" supplier={supplier} alt="tra-cuu-tien-nuoc-online" />
      ...
      {!isBillEmpty && (
        <FormProvider {...methods}>
          <EmailInput appID={AppID.WATER} />

          <GotItVoucherInput productID={ProductID.WATER} />
        </FormProvider>
      )}
